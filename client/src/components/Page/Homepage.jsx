...
import BestSell from "../common/BestSell";
...
function Homepage() {
  ...
  let [baseProducts, setBaseProducts] = useState([]);
  ...
  const getNewProduct = () => {
    axios
      ...
      .then((res) => {
        ...
        const newProducts = allProducts.filter((product) => product.new === 1);
        const latestNewProducts = newProducts.slice(-8);
        setProducts(latestNewProducts);
        setBaseProducts(allProducts.slice(8));
      })
  ...
  return (
    <div className="hero_area">
      ...
      >
        {products.map((p, i) => (
          <div key={i} className="sliderr-item">
            <NavLink
              style={{ textDecoration: "none", color: "black" }}
              to={`/product/${p.productId}`}
            >
              <img src={p.image} style={{ width: "100%", margin: "0 auto" }} />
              <h5 style={{ marginTop: "1em" }}>{p.title}</h5>
              <p style={{ color: "#db4f66" }}>${p.price}</p>
              <p>{p.content}</p>
            </NavLink>
          </div>
        ))}
